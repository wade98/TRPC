...
export async function runOffersAndMuWorkflow(
  ...
  const prices =
    ...
    (await runner.loadOrFetch("itemTrading.getPrices", {}));
  ...
  if (workOfferId) {
    await runner.loadOrFetch("workOffer.getById", { workOfferId });
  }
  ...
  const mus =
    ...
    (await runner.loadOrFetch("mu.getManyPaginated", { limit: runner.sampleSize }));

  const muId = runner.resolveId(mus, ["muId", "_id", "id"], "WARERA_MU_ID");
  ids.muId = muId;

  if (muId) {
    await runner.loadOrFetch("mu.getById", { muId });
  }
