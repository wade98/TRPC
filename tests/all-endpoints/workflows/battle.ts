...
export async function runBattleWorkflow(
  ...
  await runner.loadOrFetch("ranking.getRanking", { rankingType: "countryDamages" });
  await runner.loadOrFetch("event.getEventsPaginated", { limit: runner.sampleSize });

  const battleList =
    runner.getResponse("battle.getBattles") ??
    (await runner.loadOrFetch("battle.getBattles", { limit: runner.sampleSize }));
  ...
  for (const sampledBattleId of sampledBattleIds) {
    await runner.loadOrFetch("battle.getById", { battleId: sampledBattleId }, true);
    const battleLive = await runner.loadOrFetch(
      "battle.getLiveBattleData",
      { battleId: sampledBattleId },
      true
    );
  ...
  for (const sampledRoundId of sampledRoundIds) {
    await runner.loadOrFetch("round.getById", { roundId: sampledRoundId }, true);
    await runner.loadOrFetch("round.getLastHits", { roundId: sampledRoundId }, true);
    if (!ids.roundId) {
